...
const mainContainer = document.getElementsByClassName("who-container");
const loadingContainer = document.getElementById("loadingContainer");
const pokemonImageClass = document.getElementsByClassName("pokemonImage");
const hintElement = document.getElementById("hint-element");
const hintAbility = document.getElementById("hint-ability");

// 8.1) Initialize variables
let usedPokemonIds = []; // Array used to store the list of already used/displayed pokemon
let showLoading = true; // 16.1) Boolean used to determined HTML elements shown.
...
async function loadQuestionWithOptions() {
  ...
  hintElement.textContent = pokemon.types[0]?.type?.name;
  hintAbility.textContent = pokemon.abilities[0]?.ability?.name;
...
function hideLoadingWindow() {
  ...
  loadingContainer.classList.remove("justify-content-center");
...
function showPokemonImage() {
  pokemonImageClass[0].classList.remove("hideImage");
  pokemonImageClass[0].classList.add("showImage");
}
function hidePokemonImage() {
  pokemonImageClass[0].classList.remove("showImage");
  pokemonImageClass[0].classList.add("hideImage");
}
